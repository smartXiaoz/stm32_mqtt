...
{
    // memset(MQTT_RxDataBuf, 0, ((R_NUM-1) * BUFF_UNIT));
	...
	// memset(MQTT_TxDataBuf, 0, ((T_NUM-1) * BUFF_UNIT));
	...
	// memset(MQTT_CMDBuf, 0, ((C_NUM-1) * BUFF_UNIT));
...
/*参  数：data_len：数据长度                                */
/*返回值：无                                                */
/*temp_buff[0]=0x30,temp[1]=剩余字节长度，temp[2]=剩余字节长度，temp[3] = topic长度，temp[4] = topic长度，temp[5] = 数据，没有长度，*/
/*----------------------------------------------------------*/
...
void CMDBuf_Deal(unsigned char *data, int size)
{
	/*
		加入结束符是为了不输出后面的内容
		假设cmdbuf {{"11111"}, {"22222"}, {"33333"}};
		MQTT_CMDInPtr = cmdbuf[0] 就会输出111112222233333；
		但是假设在11111后面+\0,就不会了
	*/
